...
def main(argv):
    ...
    charter = load(argv[1]); repo = Path(argv[argv.index('--repo') + 1]) if '--repo' in argv else Path('.')
    ...
    runs = repo / '.agentic' / 'runs'; runs.mkdir(parents=True, exist_ok=True)
    ...
    intake_path = repo / '.agentic' / 'ledger' / 'intake' / f'{run_id}.json'; intake_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    if dry:
        print(json.dumps({'record': record, 'prompt': prompt}, indent=2)); return 0
